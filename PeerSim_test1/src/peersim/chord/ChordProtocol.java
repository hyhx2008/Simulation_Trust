...
public class ChordProtocol implements EDProtocol {
	...
	public void processEvent(Node node, int pid, Object event) {
		...
		if (event.getClass() == LookUpMessage.class) {
			...
			if (target != ((ChordProtocol) node.getProtocol(pid)).chordId) {
				...
				if (dest.isUp() == false) {               //如果下一跳不存在，修正chord拓扑，直到找到下一跳
					do {
						varSuccList = 0;  //???????
						stabilize(node);  //稳定化
				...
				if (dest.getID() == successorList[0].getID() //如果找到的下一跳节点已经超过了target，则表示target不存在，失败次数加一；
	...
	public void fixFingers() {
		...
		do {
			...
			pot = pot.subtract(BigInteger.ONE); // 为下次循环做准备，放宽条件
